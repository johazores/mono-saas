...
- **Notes:** `AsyncLocalStorage` provides immutable request-local scope with request ID, deployment env, verified tenant ID, and resolution source. On routes wrapped by `withRequestScope()`, T-306 produces an untrusted candidate and `resolveAuthoritativeTenant()` maps it through an active `Tenant` or owned `TenantDomain` before `tenantId` enters scope; unknown/inactive owned candidates fail closed and public `x-tenant-id` remains ignored. Member-auth routes enforce active organization membership. Adoption now covers member authentication, public products, member purchases/downloads, checkout creation/verification, and public CMS media delivery. Checkout product, active price, stored session, authenticated user, guest user/membership, and purchase paths are tenant-qualified; public media delivery tenant-qualifies the media ID before base64 delivery or storage signing. Remaining work is complete route inventory/classification, the rest of tenant-owned route adoption/policy, and real two-tenant database isolation before the Prisma guard switches from `env`.
