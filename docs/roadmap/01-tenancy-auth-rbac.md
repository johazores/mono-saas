...
- **Notes:** `AsyncLocalStorage` provides immutable request-local scope with request ID, deployment env, verified tenant ID, and resolution source. On routes wrapped by `withRequestScope()`, T-306 produces an untrusted candidate and `resolveAuthoritativeTenant()` maps it through an active `Tenant` or owned `TenantDomain` before `tenantId` enters scope; unknown/inactive owned candidates fail closed and public `x-tenant-id` remains ignored. Member-auth routes enforce active organization membership. Adoption now covers member authentication, member features, public products, member purchases/downloads, checkout creation/verification, public CMS media delivery, and SSR public CMS pages/content/block templates. Member feature resolution tenant-qualifies Membership/Feature reads and uses tenant/environment-keyed definition caches; checkout and public CMS paths are tenant-qualified before rendering, entitlement resolution, or storage signing. Remaining work is complete route inventory/classification, the rest of tenant-owned route adoption/policy, and real two-tenant database isolation before the Prisma guard switches from `env`.
...
- **Notes:** Schema-derived relation metadata scopes list `include`/`select` queries, required and optional to-one selections, relation counts, nested `connect`/`set`/`create`/`update`/`upsert`/bulk writes, and unchecked declared-relation scalar IDs by normalizing them into scoped `connect` selectors. JSON payloads are not generically walked. T-305 Stage B/C preflight/verify known soft references. Verified request tenants stamp new staged rows, and adopted product/purchase/purchase-file/product-price/checkout/media/page/content-type/content-item/block-template/membership/feature reads now add conditional tenant filters where their route context is authoritative. Feature-definition caches are isolated per tenant or deployment environment. This remains partial defense-in-depth; relation includes and remaining soft-reference routes still require the tenant-aware Prisma cutover plus T-1301 real database proof.
