...
- **Notes:** `AsyncLocalStorage` provides immutable request-local scope with request ID, deployment env, verified tenant ID, and resolution source. On routes already wrapped by `withRequestScope()`, T-306 first produces an untrusted candidate and `resolveAuthoritativeTenant()` now maps it through an active `Tenant` or owned `TenantDomain` before `tenantId` enters scope; unknown/inactive owned candidates fail closed with a generic 404 and public `x-tenant-id` remains ignored. No-candidate requests preserve deployment scope. The verified tenant is not yet membership authorization or the Prisma data guard. Remaining work is route-wide wrapper adoption, organization-membership enforcement after member identity resolution, and real concurrent two-tenant route/database isolation coverage before the Prisma guard switches from `env`.
...
- **Notes:** Schema-derived relation metadata scopes list `include`/`select` queries, required and optional to-one selections, relation counts, nested `connect`/`set`/`create`/`update`/`upsert`/bulk writes, and unchecked declared-relation scalar IDs by normalizing them into scoped `connect` selectors. JSON payloads are not generically walked. T-305 Stage B/C explicitly preflight/verify known soft references such as checkout user/product/price IDs, taxonomy parents, membership sources, feature keys, content-type slugs, legacy user hierarchy, invitations, and known activity references. Verified request tenants now stamp only newly created staged rows; reads, connects, selectors, and existing-record authorization still use `env` until the tenant-aware Prisma cutover and T-1301 real database proof.
...
- **Notes:** Stage A is merged: `Tenant`, `TenantDomain`, one-per-tenant `Organization`, `OrganizationMembership`, and provider-neutral `ExternalIdentity` foundations exist, and all 19 legacy `env`-scoped models have nullable indexed `tenantId` staging fields while retaining current `env` behavior. Stage B is merged: the explicit restartable backfill is dry-run by default, preserves `env`, validates known cross-record references, migrates membership hierarchy, and creates Clerk external identities. Stage C is merged as a strictly read-only verifier covering orphan/missing tenant assignments, canonical domain ownership, cross-record tenant consistency, workspace/identity migration, soft-key ownership, and final-index collisions. Wrapped requests can now bind candidates to active database tenants, and their verified tenant ID is stamped onto new legacy scoped rows; caller attempts to retag existing rows are discarded. Remaining work: execute backfill/verification against real data, enforce organization membership where required, run T-1301 with two real tenants, switch the Prisma guard to the accepted tenant boundary, create final indexes, complete global-user/provider-neutral identity cutover, and only then remove legacy scope fields.
...
- **Notes:** Added subdomain, custom-domain, path-prefix, and HMAC-signed trusted-header candidate strategies. Global unscoped `SystemConfig.TENANT_RESOLUTION` selects exactly one mode and is cached for five seconds; the trusted-header secret comes only from `TENANT_RESOLUTION_SHARED_SECRET`, never from database JSON. Host/path/key normalization, timestamp skew, signature comparison, replay binding, and public-header spoofing are covered by focused tests. Resolution still returns only an untrusted candidate key; the T-301/T-305 request boundary now performs the separate authoritative `Tenant`/`TenantDomain` lookup before trusted context is established.
