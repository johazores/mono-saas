...
- **Notes:** `AsyncLocalStorage` provides immutable request-local scope with request ID, deployment env, verified tenant ID, and resolution source. On routes already wrapped by `withRequestScope()`, T-306 first produces an untrusted candidate and `resolveAuthoritativeTenant()` maps it through an active `Tenant` or owned `TenantDomain` before `tenantId` enters scope; unknown/inactive owned candidates fail closed with a generic 404 and public `x-tenant-id` remains ignored. No-candidate requests preserve deployment scope. Current member-auth routes now require an existing active `OrganizationMembership` for returning credential/Clerk identities, while genuinely new authorized registrations/provisioning create membership in the resolved one-per-tenant organization. Concurrent wrapper tests assert two authoritative tenant contexts do not bleed across requests. Remaining work is route-wide wrapper/membership-policy adoption and real two-tenant database isolation coverage before the Prisma guard switches from `env`.
