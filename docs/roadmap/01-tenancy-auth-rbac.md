...
- **Notes:** Stage A/B/C are merged: tenant/workspace/external-identity foundations, explicit restartable backfill, and the read-only cutover verifier exist while all legacy `env` fields remain active. Wrapped requests bind candidates to active database tenants, new scoped rows receive trusted tenant IDs, update-time retagging is discarded, and current member-auth users are membership-checked/provisioned safely. Route adoption now includes public products, member purchases/downloads/features/sub-users, checkout, public CMS media delivery, and SSR public CMS page/content/template reads. Runtime SiteSetting reads reject another tenant's staged config/secret rows, while tenant-bound writes are intentionally blocked until `SiteSetting(tenantId,key)` can replace legacy `SiteSetting(env,key)`. Remaining work: execute migration verification on real data, make remaining tenant-owned/soft-reference routes safe, run T-1301 with two real tenants, switch the Prisma guard to the accepted tenant boundary, create final indexes, complete global-user/provider-neutral identity cutover, and only then remove legacy scope fields.
