...
- **Notes:** `AsyncLocalStorage` provides immutable request-local scope with request ID, deployment env, verified tenant ID, and resolution source. On routes wrapped by `withRequestScope()`, T-306 produces an untrusted candidate and `resolveAuthoritativeTenant()` maps it through an active `Tenant` or owned `TenantDomain` before `tenantId` enters scope; unknown/inactive owned candidates fail closed and public `x-tenant-id` remains ignored. Member-auth routes enforce active organization membership. Adoption now covers public auth/site/payment configuration, public products, public credential-invitation acceptance, member authentication/billing/profile/purchases/downloads/features/sub-users, checkout creation/verification, public CMS media delivery, and SSR public CMS pages/content/block templates. Tenant-bound invitation acceptance requires the token row to be staged to the verified tenant and uses rollback-safe workspace registration; it deliberately does not infer the future organization/role invitation target from `env`. Public/runtime configuration reads use tenant-qualified SiteSetting rows and auth/Clerk/payment/storage/site caches are keyed by verified tenant or deployment environment, so cached configuration cannot bleed across workspaces. The profile route gates tenant-bound member access while preserving global User identity fields; tenant-specific plan enrichment still comes from tenant-qualified purchase/product reads. Tenant-bound setting writes remain blocked until the final tenant-aware unique index exists. Billing sync throttling is tenant+user scoped with environment fallback, while `User.stripeCustomerId` remains transitional T-702 debt rather than final multi-workspace billing architecture. Remaining work is complete route inventory/classification, the rest of tenant-owned route adoption/policy, and real two-tenant database isolation before the Prisma guard switches from `env`.
...
- **Notes:** Schema-derived relation metadata scopes list `include`/`select` queries, required and optional to-one selections, relation counts, nested `connect`/`set`/`create`/`update`/`upsert`/bulk writes, and unchecked declared-relation scalar IDs by normalizing them into scoped `connect` selectors. JSON payloads are not generically walked. T-305 Stage B/C preflight/verify known soft references. Verified request tenants stamp new staged rows, and adopted product/purchase/purchase-file/product-price/checkout/media/page/content-type/content-item/block-template/membership/feature/SiteSetting/UserInvitation reads now add conditional tenant ownership checks where request context is authoritative. SiteSetting-backed auth, Clerk security, payment, storage, and site/theme caches are isolated by tenant/environment scope as well as repository filtering. Invitation acceptance token/status operations are tenant-qualified and registration rollback removes partial Membership/Purchase/User data if free-plan or workspace provisioning fails. Member profile plan enrichment reuses the tenant-qualified purchase/product chain while global User identity fields stay global. Legacy sub-user hierarchy reads are tenant-qualified, and create/link/revoke mirrors parent/ancestor relationships into membership IDs with compensation if one side fails. Billing provider sync consumes tenant-qualified product/purchase/config data and uses a scope-aware throttle. This remains partial defense-in-depth; relation includes and remaining soft-reference routes still require the tenant-aware Prisma cutover plus T-1301 real database proof.
...
- **Notes:** Stage A/B/C are merged: tenant/workspace/external-identity foundations, explicit restartable backfill, and the read-only cutover verifier exist while all legacy `env` fields remain active. Wrapped requests bind candidates to active database tenants, new scoped rows receive trusted tenant IDs, update-time retagging is discarded, and current member-auth users are membership-checked/provisioned safely. Route adoption now includes public auth/site/payment configuration, public credential-invitation acceptance, public products, member billing/profile/purchases/downloads/features/sub-users, checkout, public CMS media delivery, and SSR public CMS page/content/template reads. Tenant-bound invitation tokens/status are scoped to staged `UserInvitation.tenantId`, accepted users use the same workspace membership path as open registration, and rollback removes partial registration data on provisioning failure. Public/runtime SiteSetting reads and their in-process caches reject cross-tenant config/secret reuse, the profile route preserves the accepted global-User identity boundary while tenant-specific plan state remains workspace-scoped, billing local sync state is tenant-aware, and live sub-user mutations mirror the Stage B membership hierarchy mapping. Remaining work: execute migration verification on real data, make remaining tenant-owned/soft-reference routes safe, run T-1301 with two real tenants, switch the Prisma guard to the accepted tenant boundary, create final indexes, complete global-user/provider-neutral identity and billing-reference cutovers, complete explicit T-602 organization/role invitation targeting, and only then remove legacy scope fields.
...
- **Notes:** Returning linked users resolve from the local database using the verified Clerk subject. Email/name claims are used when present. A cached Clerk profile lookup occurs only when a new identity needs linking/provisioning and required claims are absent; the fallback profile calls are cached.
